...
`@napi-rs/simple-git-linux-x64-gnu@0.1.16`は、標準環境でNextraを実行するために必要なLinux x64 glibc向けnpmパッケージとして、`package.json`のoptional dependencyと`package-lock.json`へ記録します。これはnpmパッケージのため`devbox.lock`では管理しません。
