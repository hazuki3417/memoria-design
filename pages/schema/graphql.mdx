...
> 状態: 初期設計を含む検討資料  
> 現行の実行可能なGraphQL schemaは `memoria-api/schemas/graphql/*.graphql` を正本とします。  
> 現行契約と実装状況は[実装・設計整合状況](../implementation-status)を参照してください。このページのContent中心の例は、未実装または旧設計の可能性があるため、現行APIとして扱いません。
