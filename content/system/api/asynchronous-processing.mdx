...
- Workerはmessage受信後に対象Processing Jobをrow lockし、`PENDING`なら`PROCESSING`へ遷移して`attempts`を増加させてcommitしてからPipelineを実行します。terminalな`COMPLETED` / `FAILED`のduplicate deliveryはPipelineを再実行せず`XACK`します。
