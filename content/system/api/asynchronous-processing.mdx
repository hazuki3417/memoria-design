...
Image Variantは履歴やgenerationをDBへ保持せず、`image_variants`をその時点で有効な状態の正本とします。再処理では既存Variantを提供し続け、新Variant群を別のObject Storage keyへ生成します。全必要Variantの生成成功後だけ、対象Mediaを`FOR UPDATE`でlockする短いPostgreSQL transactionで`ACTIVE`を再確認し、ACTIVEの場合だけVariant metadataを一括置換してProcessing Jobの完了を同時に確定します。削除要求が先に`DELETING`をcommitしていた場合はpublicationせずJobを`CANCELLED`とし、生成済み未公開ObjectをStorage OperationのDELETE対象にします。publicationが先にMedia lockを取得してcommitした場合、後続の削除要求は新しくpublicationされたVariantを含む最新状態から削除対象を確定します。
...
未公開Variantのstaging Object一覧をProcessing Jobや別relationへ永続化しません。staging Object keyはMedia IDとJob IDから対象を識別できるnamespace規約とし、Workerが`DELETING`を検出した場合は自身が生成済みの未公開keyについてStorage Operationをdurableに作成してからJobを`CANCELLED`へ確定します。Object生成後からcleanup operation記録前にWorkerが停止するfailure windowは、Object Storage orphan reconciliationを最終的な安全網として回収します。reconciliationはpublished VariantとしてDBから参照されず、かつactive Jobに対応しないstaging Objectをgrace period後のcleanup候補として扱います。走査間隔とgrace periodの具体値はruntime / operations設計で確定します。
