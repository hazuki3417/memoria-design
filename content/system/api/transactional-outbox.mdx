...
`next_attempt_at`は`PENDING`では実行可能時刻、`PROCESSING`ではclaim lease期限として統一して使用します。新規`PENDING` eventは原則`created_at`を設定します。Publisherは`status IN ('PENDING', 'PROCESSING') AND next_attempt_at <= now()`を対象にし、`created_at, id`で安定順序を付けます。

`PUBLISHED` eventは障害調査のため`published_at`から7日間保持し、その後batchで物理削除します。未配送・処理中・運用対応待ちである`PENDING` / `PROCESSING` / `FAILED`は自動cleanupしません。Outbox retentionは利用者操作traceであるMedia Eventの保持方針とは独立です。
...
- 配送messageは`event_id`、`event_type`、`event_version`、`aggregate_id`、`payload`からなるenvelopeとし、payload契約は`event_type + event_version`で識別します。
- `event_version`は1から開始し、互換性のないpayload変更時に増加させます。Publisherは保存済みpayloadを新versionへ変換せず、そのまま配送します。
- deploy時に未配送の旧version eventが残り得るため、Workerは残存し得る旧versionを処理できる互換期間を持たせます。
