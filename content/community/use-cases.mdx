...
| Actor | 役割 |
| --- | --- |
| User | Communityを作成し、Invitationから参加する |
| Administrator | Community設定、Invitation、Membership role、参加者、Community削除を管理する |
| Member | Communityへ参加し、Mediaの閲覧・Upload・分類を行う |
| Uploader | Community管理MediaをUploadし、参加中に限り本人向けMedia権限を持つ |
...
- 期限切れ・無効化後は新しいInvitation acceptを拒否する。
- Community削除後はURLを利用できず、名称を含むCommunity情報を表示しない。
- HTTPSを必須とし、tokenをlog、外部解析、第三者Serviceへ送信しない。
- URL確認とInvitation acceptにはrate limitを適用する。具体値はSecurity要件で決定する。
- login後の戻り先はアプリケーション内で許可されたpathだけを受け付ける。

期限切れは`expires_at`の比較で判定し、状態を更新する常駐処理は必須としません。

## InvitationからCommunityへ参加する

Community外の登録済みUserが、有効なInvitation URLを使って実行します。未登録Userはloginまたは登録後、元の招待画面へ戻ります。参加成立前に表示するCommunity情報は名称だけとし、Member一覧、Media、Group、Tag、メールアドレスは表示しません。

Invitation acceptではURLの有効性、Communityの存在、Membershipが存在しないことを確認し、Invitationの有効性をlock後に再確認し、`role=member`のMembership作成を同じTransactionで確定します。Invitation自体にUserごとの使用済み状態は記録しません。参加済みUser、期限切れ・無効化済みInvitation、削除済みCommunityでは新しいMembershipを作成しません。

Initial ReleaseではInvitation acceptにAdministrator承認を挟まず、Join Requestも作成しません。このflowの正本は[Community論理データモデル](./data-model)と[Communityの整合性とTransaction](./consistency-transactions)です。

## User起点Join Request

Invitationとは独立した将来機能です。Communityと申請Userを参照し、`PENDING`、`APPROVED`、`REJECTED`、`CANCELLED`、`INVALIDATED`の状態を持ちます。Initial ReleaseではUI/APIを提供しません。
...
Communityから除外されたUserも、有効な招待URLがあれば再度Invitationをacceptできます。Invitationが有効でMembershipが存在しなければ、Administratorの個別承認なしで`role=member`として再参加します。
...
- User起点Join Requestは将来機能とし、Initial ReleaseではUI/APIを提供しません。
- メール・push通知は[Issue #47](https://github.com/hazuki3417/memoria-design/issues/47)で将来検討します。
- Media管理主体の委譲は[Issue #48](https://github.com/hazuki3417/memoria-design/issues/48)で将来検討します。
- Community説明、公開検索、User block、一括承認・拒否・一括委譲はInitial Releaseに含めません。
- file本体の差し替えと、実際の編集項目がないMedia編集画面はInitial Releaseに含めません。
