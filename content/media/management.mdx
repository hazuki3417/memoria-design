...
Media本体はメディア情報だけを表し、管理主体は独立した`media_custodies`で関連付けます。個人領域へのuploadではUser、Communityへの直接uploadではCommunityが初期Custodianです。Communityへ直接uploadしたUploader本人には、有効なMembershipを持つ間に限り、自分がUploadしたCommunity管理MediaのOriginal downloadと削除を許可します。この限定権限はCustodyそのものではなく、退会・除外・User削除で失効します。
