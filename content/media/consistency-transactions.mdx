...
| --- | --- |
| upload | Media、subtype、Custody、UPLOADED eventの作成 |
| 公開・解除 | Custody確認、Access追加・削除、Event作成 |
...
- 委譲と公開が競合した場合、先にcommitした結果を後続処理が再検証
- DELETING Mediaへの公開・委譲・Group追加を拒否
- 同じCommunityへの同時公開は複合PKで重複を防止
...
2. ULIDのMedia IDを生成
3. Object Storageの最終keyへ原本と必要な派生物をupload
4. DB transactionでMedia、subtype、Custody、Eventを作成
...
3. Custodian、Community Membership、AdministratorをDBで確認
4. 操作別の権限を判定
5. 更新対象をlockした後に権限を再確認

clientが指定したUser IDやCommunity IDだけを根拠にしません。

## Errorと再試行

- constraint違反はdomain errorへ変換し、DB詳細を公開しない
