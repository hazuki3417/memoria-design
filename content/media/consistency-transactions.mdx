# Mediaの整合性とTransaction

## 概要

> 実装状況: ImageのDB・Object Storage操作は存在しますが、Custody・Access・Eventとstorage operationを含むtransaction設計は未実装です。

Transaction境界、lock順序、認可の再評価、外部I/Oの扱いは[Transaction・競合制御の共通方針](/system/cross-cutting/transactions-consistency)に従います。本ページではMedia固有の更新対象と競合条件を定義します。

## Transaction境界

| ユースケース | DB transactionに含める処理 |
| --- | --- |
| upload | Media、subtype、Custody、UPLOADED event、Processing Job、Outbox Eventの作成 |
| 共有・解除 | Custody確認、Access追加・削除、Event作成 |
| 委譲 | Media lock、Custody lock、全Access削除、Custody更新、Event作成 |
| Group追加・解除 | scope・権限・循環確認、関連更新 |
| 削除要求 | ACTIVE確認、DELETING更新、storage operation作成 |
| 削除完了 | domain関連削除、Media削除、DELETED event作成 |

UserとCommunityが所有するTransactionは、それぞれ[Userの整合性とTransaction](/user/consistency-transactions)、[Communityの整合性とTransaction](/community/consistency-transactions)を正本とします。

## 競合制御

### 委譲・共有・削除

Media lifecycleの直列化点は`media`行とし、Custodian変更・共有可否の現在状態は`media_custodies`行を正本とします。共有・解除・委譲・削除要求では対象`media`を`SELECT ... FOR UPDATE`でlockし、続いて`media_custodies`をlockします。lock取得後にMediaがACTIVEであること、現在Custodian、操作者権限を再確認します。

- 委譲と共有が競合した場合、先にcommitした結果を後続処理が再検証
- DELETING Mediaへの共有・委譲・Group追加を拒否
- 同じCommunityへの同時共有は複合PKで重複を防止
- update件数が期待値と異なる場合は競合errorとしてrollback

### Group DAG

Group DAG変更は同一scope単位で直列化します。User scopeでは対象User行、Community scopeでは対象Community行をscopeの排他点としてlockし、scopeと権限を再確認した後、追加するchildからparentへ到達できるかをrecursive CTEで確認してrelationを更新します。cycle確認とinsertは同一transactionで行います。これにより、互いの未commit relationを参照できない並行追加によってcycleが成立することを防ぎます。Initial Releaseでは同一scope内の独立したGroup階層変更も直列化し、複雑なgraph単位lockは導入しません。

## Upload

初期構成ではFrontendがAPIへOriginalを送信し、APIがObject Storageへ保存します。Frontendから署名付きURLでS3へ直接uploadする方式は、APIまたはEC2の通信負荷が問題になった場合の将来的な最適化候補であり、現在のupload契約には含めません。

1. APIでfile size、MIME type、実dataを検査
2. ULIDのMedia IDを生成
3. Object Storageの最終keyへOriginalだけをupload
4. DB transactionでMedia、subtype、Custody、Media Event、Processing Job、Outbox Eventを作成
5. DB transaction失敗時はupload済みobjectを冪等に削除
6. 補償削除に失敗した場合は運用retry対象として記録

DBへ先に登録しないのは、利用可能なMedia行がfileなしで残ることを避けるためです。Objectだけが残る失敗は一覧取得へ影響せず、補償削除できます。

個人領域へのuploadではUser、Communityへの直接uploadではCommunityを初期Custodianとします。派生物はAPI request内で生成せず、commit後にOutbox PublisherがRedis Streamsへ通知し、Media Workerが非同期で生成します。APIからRedisへ直接送信しません。

将来S3への直接uploadを採用する場合は、署名付きURLの発行権限、完了通知、実dataの検証、未確定Objectの削除、Idempotency Key、DB transaction境界を改めて設計します。複数fileの選択やURLの一括発行を導入する場合も、retryと非同期処理の基本単位は1 Mediaのまま維持します。

## Delete

削除は復元を提供しませんが、DBとObject Storageの部分失敗を放置しないため一時的なDELETING状態を使用します。

```mermaid
sequenceDiagram
  actor User
  participant API
  participant DB
  participant Worker
  participant Storage
  User->>API: Media削除
  API->>DB: DELETING・削除operation作成
  API-->>User: 削除受付
  Worker->>Storage: objectを冪等に削除
  Worker->>DB: operation完了
  Worker->>DB: Media関連を物理削除・Event作成
```

- Objectが存在しない場合も削除成功として扱います。
- 一部objectの削除失敗は未完了operationだけをretryします。
- DELETING中のMediaは閲覧・更新対象から除外します。
- 削除要求transactionでは、その時点でDBから確定できるOriginalとpublished VariantのkeyをStorage Operationとしてsnapshotします。実行中Processing Workerが後から`DELETING`を検出した場合は、そのWorkerが生成済み未公開ObjectのStorage Operationを追加してJobを`CANCELLED`にします。
- Media削除に関連してdurableに記録されたStorage Operationがすべて`COMPLETED`であることを確認した後、最後のOperation完了確定、DELETED Event作成、domain関連削除、Processing Jobの明示削除、Media物理削除を一つのDB transactionで確定します。`PENDING` / `PROCESSING` / `FAILED`が残る間はMediaを`DELETING`のまま保持し、特に`FAILED`は運用対応待ちとします。
- 完了したStorage OperationはMedia物理削除後も運用・障害調査用に一定期間保持し、削除完了transactionでは削除しません。保持期間とcleanup intervalは運用設計で別途確定します。
- backupは障害復旧用であり、個別Mediaの復元には使用しません。

## Event

現在状態のCustodyと操作traceを同じtableへ混在させません。

- `media_custodies`: 現在の管理主体
- `media_events`: 誰がどの操作をしたか
- `outbox_events`: 確定した変更をWorkerへ確実に伝達するための内部event
- application log・trace: 障害調査

Event作成は対象のdomain変更と同じDB transactionで行います。Eventが作成できない場合はdomain変更もrollbackします。

## 認可確認

認可は次の順で判定します。

1. 認証済みUserをcontextから取得
2. Media statusがACTIVEであることを確認
3. Custodian、Community Membership、Administrator、UploaderをDBで確認
4. 操作別の権限を判定
5. 更新対象をlockした後に権限を再確認

clientが指定したUser IDやCommunity IDだけを根拠にしません。Community管理MediaのOriginal download・削除ではAdministrator、または有効なMembershipを持つUploader本人であることを確認します。共有されたUser管理Mediaに対してAdministrator権限を拡張しません。

## 外部I/Oと失敗時処理

Object StorageへのOriginal保存・削除とDB更新は単一Transactionにできないため、upload時はDB失敗後の補償削除、削除時は`media_storage_operations`による冪等retryで最終整合性を保証します。派生物生成はDB commit後にOutboxを介してWorkerへ配送し、API成功条件にRedis配送完了を含めません。

Storage Operationの実行ではObject Storage I/O中にPostgreSQL row lockを保持しません。Workerは実行可能な`PENDING`またはlease期限切れ`PROCESSING`を`FOR UPDATE SKIP LOCKED`でclaimし、`PROCESSING`とlease期限をcommitしてからDELETEします。retry可能failureは指数backoffで`PENDING`へ戻し、上限到達または回復不能failureは`FAILED`として運用対応待ちにします。claim後の停止はlease期限切れoperationの再取得で回復します。

## 他domainとの境界

Community作成、Membership role、Invitation、User起点Join Request、退会・除外、Community削除のCommunity側Transactionは[Communityの整合性とTransaction](/community/consistency-transactions)を正本とします。User削除受付とUser状態は[Userの整合性とTransaction](/user/consistency-transactions)を正本とします。Media domainはCommunityとの共有、Media Custody、Upload、Media削除、Object Storageとの整合性を保証します。

## エラーと境界条件

- constraint違反はdomain errorへ変換し、DB詳細を共有しない
- deadlock・serialization failureは上限回数を設けてretry
- storage retryは指数backoffと最大間隔を設定
- retry回数だけで破棄せず、FAILEDとして運用から確認可能にする
- logへtoken、object内容、署名付きURL、個人情報を出力しない

## Test観点

各操作は、対象レコードだけでなく対象外レコードが変化していないことを確認します。

- transaction途中のerrorで部分更新が残らない
- 同時委譲でCustodianが複数にならない
- 委譲後にAccessが残らない
- Community CustodyへAccessを作れない
- Groupの多段循環を拒否する
- storage retryで二重削除errorにならない
- Media削除後も必要期間のEventを検索できる
