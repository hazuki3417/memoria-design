...
この責務分離により、Processing Jobの取得indexをDB Worker queueとして設計しません。Redis data喪失等のreconciliationではactive Jobを検索して再enqueueできるindexを別途最終化します。retry backoffの待機方式はStep 10で引き続き確定します。失敗情報に署名付きURL、credential、個人情報を保存しません。
