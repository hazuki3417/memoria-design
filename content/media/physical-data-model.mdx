...
`(image_media_id, format, width)`にUNIQUE constraintを設定し、同じ要求サイズ・formatのVariantを重複作成しません。同じJobの再実行でもこのconstraintによって重複を防止します。動画派生物は要件が具体化した時点で別設計として追加し、Image Variantへ混在させません。
...
| status | varchar(16) | NOT NULL、CHECK | `PENDING`、`PUBLISHED`、`FAILED` |
...
`status`と`published_at`の整合はCHECK constraintで保証し、`PUBLISHED`の場合に限って`published_at`をNOT NULLとします。`PENDING` / `FAILED`では`published_at`をNULLとします。`last_error`はretry policyの詳細確定前にstatusとの必須関係を設けません。

`(status, created_at, id)`をindex対象とし、Publisherが未送信eventを安定した順序で取得します。payloadへcredential、署名付きURLなどのsecretを保存しません。
...
`(actor_user_id, operation, idempotency_key)`へUNIQUE constraintを設定し、同じkeyで異なる`request_hash`を受け付けません。`(expires_at)`をcleanup用indexとします。response_dataへ不要な個人情報やsecretを保存しません。保存期間は再送要件と個人情報の保持方針に従います。
...
PKは`(media_id, tag_id)`です。Tag削除時は関連付けだけを削除し、Media本体を削除しません。TagのscopeとMediaの操作・共有状態が整合することはServiceで検証します。User管理MediaのCommunity共有解除時は、そのCommunity scopeのTag関連だけを削除します。
...
`(media_id, operation_type, object_key)`へUNIQUE constraintを設定し、`(status, next_attempt_at)`を再試行対象取得用indexとします。このtableは復元機能ではなく、削除を最終的に完了させるために使用します。
