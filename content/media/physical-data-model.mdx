...
| next_attempt_at | timestamptz | NULL可 | retry予定時刻。PROCESSINGではclaim lease期限 |
| updated_at | timestamptz | NOT NULL | 最終更新時刻 |

`status`と`published_at`の整合はCHECK constraintで保証し、`PUBLISHED`の場合に限って`published_at`をNOT NULLとします。`PENDING` / `PROCESSING` / `FAILED`では`published_at`をNULLとします。`PROCESSING`では`next_attempt_at`をclaim lease期限として必須とし、期限到達後は別Publisherが回収できます。`FAILED`は自動retry上限到達後のterminal operational stateとし、自動取得対象から外します。運用再開時は`PENDING`へ戻してretry可能状態にします。
...
Publisher取得用indexはstatus、実行可能時刻、安定したtie-breakerを含む形をStep 9で最終確定します。payloadへcredential、署名付きURLなどのsecretを保存しません。
