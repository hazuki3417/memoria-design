...
| original_storage_key | text | NOT NULL、UNIQUE | Object Storageへ保存したOriginalの実key |
...
`original_storage_key`はUploadを担当するApplicationがObject Storage保存前に生成し、実際に使用したkeyを保存します。keyの具体的なnamespace / formatはStorage実装設計で定義し、Media IDからの決定論的導出をDB契約にはしません。削除要求ではこの保存値をOriginalの削除対象keyとしてsnapshotします。
...
| --- | --- | --- | --- |
| media_id | varchar(26) | PK、FK media ON DELETE CASCADE | 対象Media ID |
| width | integer | NOT NULL、CHECK 1以上 | 動画の幅 |
| height | integer | NOT NULL、CHECK 1以上 | 動画の高さ |
...
| width | integer | NOT NULL、CHECK 1以上 | 派生画像の幅 |
| height | integer | NOT NULL、CHECK 1以上 | 派生画像の高さ |
...
| status | varchar(16) | NOT NULL、CHECK | `PENDING`、`PROCESSING`、`COMPLETED`、`FAILED`、`CANCELLED` |
| attempts | integer | NOT NULL、CHECK 0以上 | 実行試行回数 |
| last_error | text | NULL可 | 内部向けの失敗理由 |
| created_at | timestamptz | NOT NULL | 登録時刻 |
| updated_at | timestamptz | NOT NULL | 更新時刻 |

`PENDING` / `PROCESSING`をactive Jobとし、同一`(media_id, job_type)`ではactive Jobを1件だけ許可します。これは`WHERE status IN ('PENDING', 'PROCESSING')`のpartial unique indexでDB上も保証します。同じ処理のretryは新Jobを作らず同じ`job_id`を使用し、明示的なreprocessは既存Jobが`COMPLETED`、`FAILED`または`CANCELLED`のterminal stateになった後に新しいJobを作成します。`CANCELLED`はMedia削除により処理が意図的に不要になった状態であり、処理障害を表す`FAILED`とは区別します。並行したreprocess要求がactive Job作成で競合した場合は、unique constraintを境界として既存active Jobを採用し、同一処理を重複開始しません。

Processing Jobは処理状態と試行結果のdurable truthですが、実行中messageのownership / crash recovery leaseはRedis Consumer Groupのpending stateを正本とします。Processing Jobへ`lease_expires_at`等の重複lease columnは追加しません。WorkerはRedis messageを取得・claimした後にJobをrow lockし、`PENDING`から`PROCESSING`への開始または`PROCESSING` Jobのretryを確定します。`attempts`はPipelineの実行開始ごとに増加させます。`COMPLETED` / `FAILED` / `CANCELLED`へのduplicate deliveryは再実行しません。Processing開始時とVariant publication時には対象Mediaをlockして`ACTIVE`を再確認し、`DELETING`ならpublicationせずJobを`CANCELLED`として確定します。

この責務分離により、Processing Jobの取得indexをDB Worker queueとして設計しません。一時failureはRedis Consumer Groupのpendingへ残し、固定backoff相当のidle時間経過後に`XAUTOCLAIM`で同じJobをretryします。Initial ReleaseではProcessing Jobへ`next_attempt_at`を追加せず、retry回数上限とreclaim idle時間はruntime parameterとしてIssue #44 / #61で確定します。

Redis data喪失等のreconciliationではactive Jobを検索して再enqueueするため、`(status, updated_at, id)`をreconciliation用indexとします。通常のWorker実行順序をこのindexで管理しません。失敗情報に署名付きURL、credential、個人情報を保存しません。
...
| from_user_id | varchar(26) | NULL可、FK users ON DELETE SET NULL | 委譲元のUser ID |
| to_user_id | varchar(26) | NULL可、FK users ON DELETE SET NULL | 委譲先のUser ID |
| from_community_id | varchar(26) | NULL可、FKを設定しない | 委譲元Communityのsnapshot ID |
| to_community_id | varchar(26) | NULL可、FKを設定しない | 委譲先Communityのsnapshot ID |
...
`(media_id, operation_type, object_key)`へUNIQUE constraintを設定します。`PENDING` / `PROCESSING`では`next_attempt_at`を必須とし、`COMPLETED` / `FAILED`ではNULLとします。新規`PENDING`は原則即時実行可能とし、`PROCESSING`では`next_attempt_at`をclaim lease期限として使用します。

Storage Workerは`status IN ('PENDING', 'PROCESSING') AND next_attempt_at <= now()`を`FOR UPDATE SKIP LOCKED`でbatch取得し、短いtransactionで`PROCESSING`とlease期限を設定してcommitした後にObject Storage DELETEを実行します。Objectが既に存在しない場合も成功として扱います。成功後は別transactionで`COMPLETED`へ遷移します。retry可能failureでは`attempt_count`を増加し、指数backoffと最大間隔から算出した次回実行時刻を設定して`PENDING`へ戻します。retry上限到達または回復不能failureでは`FAILED`とし、自動取得対象から外します。運用再開時は`PENDING`へ戻して新しい`next_attempt_at`を設定します。claim後のWorker停止はlease期限切れ`PROCESSING`の再取得で回復します。

取得indexは`(status, next_attempt_at, created_at, id)`とします。lease時間、retry上限、backoff初期値・最大値はruntime parameterとし、DB schemaの固定値にはしません。このtableは復元機能ではなく、削除を最終的に完了させるために使用します。`FAILED`は削除未完了・整合性未収束の運用対応待ちなので自動cleanupしません。完了したoperationはMedia物理削除と同時には削除せず、運用・障害調査用の履歴として一定期間保持した後にcleanupします。具体的な保持期間とcleanup intervalは運用設計で確定します。
